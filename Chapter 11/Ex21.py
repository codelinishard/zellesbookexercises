#I preferred circle targets
#for a rectangular target, the formula for checking hits is
# if x>= min and x <= max and y >= min and y<= max
# where x and y are the center coordinate of the cannonball
from math import sqrt, sin, cos, radians, degrees
from graphics import *
from projectile import Projectile
from random import *

class Launcher:

    def __init__(self, win):
        """Create inital launcher with angle 45 degrees and velocity 40
        win is the GraphWin to draw the launcher in.
        """
        
        # draw the base shot of the launcher
        self.base = Circle(Point(0,0), 3)
        self.base.setFill("red")
        self.base.setOutline("red")
        self.base.draw(win)

        # save the window and create initial angle and velocity
        self.win = win
        self.angle = radians(45.0)
        self.vel = 40.0
        
        # create inital "dummy" arrow
        self.arrow = Line(Point(0,0), Point(0,0)).draw(win)
        # replace it with the correct arrow
        self.redraw()

        
    def redraw(self):
        """undraw the arrow and draw a new one for the
        current values of angle and velocity.
        """
        
        self.arrow.undraw()
        pt2 = Point(self.vel*cos(self.angle)+ self.base.getCenter().getX(), self.vel*sin(self.angle)+ self.base.getCenter().getY())
        self.arrow = Line(self.base.getCenter(), pt2).draw(self.win)
        self.arrow.setArrow("last")
        self.arrow.setWidth(3)

    def adjHeight(self, amt):
        """ change height by amt Height """

        self.base.move(0,amt)
        self.redraw()
   
    def adjAngle(self, amt):
        """ change angle by amt degrees """
        
        self.angle = self.angle+radians(amt)
        self.redraw()

        
    def adjVel(self, amt):
        """ change velocity by amt"""
        
        self.vel = self.vel + amt
        self.redraw()

    def fire(self):
        return ShotTracker(self.win, degrees(self.angle), self.vel, self.base.getCenter().getY())
  

class ShotTracker:

    """ Graphical depiction of a projectile flight using a Circle """

    def __init__(self, win, angle, velocity, height):
        """win is the GraphWin to display the shot, angle, velocity, and
        height are initial projectile parameters.
        """
        
        self.proj = Projectile(angle, velocity, height)
        self.marker = Circle(Point(0,height), 3)
        self.marker.setFill("red")
        self.marker.setOutline("red")
        self.marker.draw(win)

        
    def update(self, dt):
        """ Move the shot dt seconds farther along its flight """
        
        self.proj.update(dt)
        center = self.marker.getCenter()
        dx = self.proj.getX() - center.getX()
        dy = self.proj.getY() - center.getY()
        self.marker.move(dx,dy) 
    def getX(self):
        """ return the current x coordinate of the shot's center """
        return self.proj.getX()
    def getY(self):
        """ return the current y coordinate of the shot's center """
        return self.proj.getY()
    def undraw(self):
        """ undraw the shot """
        self.marker.undraw()

class Target:
    def __init__(self,win,x,y,radius):
        self.center = Point(x,y)
        self.cX = x
        self.cY = y
        self.radius = radius
        self.trackedhits = 0
        self.location = Circle(Point(x,y),radius)
        self.location.setFill("green")
        self.location.setOutline("green")
        self.location.draw(win)

    def move(self,x,y):
        self.location.move(x,y)
        self.cX += x
        self.cY += y
        self.center = Point(self.cX,self.cY)

    def checkhit(self,coordinates):
        x = coordinates.getX()
        y = coordinates.getY()
        if (self.cX - x)**2 + (self.cY - y)**2 <= self.radius^2:
            self.trackedhits += 1
            return True
        else: return False

    def generateNew(self, minX,minY,maxX,maxY):
        x = randrange(minX,maxX+1)
        y = randrange(minY,maxY+1)
        self.move(x-self.cX,y-self.cY)
        
    def checkBoundary(self,minX,minY,maxX,maxY):
        if self.cX >= maxX or self.cX <= minX:
            return True
        elif self.cY >= maxY or self.cY <= minY:
            return True
    



        


class ProjectileApp:
    def __init__(self):
        self.win = GraphWin("Projectile Animation", 640, 480)
        self.win.setCoords(-10, -10, 210, 155)
        Line(Point(-10,0), Point(210,0)).draw(self.win)
        for x in range(0, 210, 50):
            Text(Point(x,-7), str(x)).draw(self.win)
            Line(Point(x,0), Point(x,2)).draw(self.win)

        self.launcher = Launcher(self.win)
        self.target = Target(self.win,100,100,5)
        self.shots = []
        self.display = Text(Point(110,83),f"{self.target.trackedhits} targets hit")
        self.display.draw(self.win)
    def updateShots(self, dt):
        alive = []
        for shot in self.shots:
            shot.update(dt)
            if shot.getY() >= 0 and shot.getX() < 210:
                alive.append(shot)
            else:
                shot.undraw()
            if self.target.checkhit(shot.marker.getCenter()):
                self.display.undraw()
                self.display.setText(f"{self.target.trackedhits} targets hit")
                self.display.draw(self.win)
                self.target.generateNew(40,40,170,115)
        self.shots = alive

    def run(self):
        
        # main event/animation lopp
        while True:
            self.updateShots(1/30)
            self.target.move(randrange(-1,2),randint(-1,2))
            if self.target.checkBoundary(0,0,200,145):
                self.target.generateNew(20,20,170,115)
            key = self.win.checkKey()
            if key in ["q", "Q"]:
                break

            if key == "Up":
                self.launcher.adjAngle(5)
            elif key == "Down":
                self.launcher.adjAngle(-5)
            elif key == "Right":
                self.launcher.adjVel(5)
            elif key == "Left":
                self.launcher.adjVel(-5)
            elif key == "f":
                self.shots.append(self.launcher.fire())
            elif key == "u":
                self.launcher.adjHeight(1)
            elif key == "d":
                self.launcher.adjHeight(-1)
            
            update(30)
            
        self.win.close()
           

if __name__ == "__main__":
    ProjectileApp().run()
